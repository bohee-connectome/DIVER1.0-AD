...
class utils():
    ...
    def _load_xyz_from_elc(elc_path: str,
        ...
        if positions_start is None or labels_start is None:
            raise RuntimeError("No Position/Labels section in elc file.")
...
class ADFTDDatasetSetting(DefaultDatasetSetting):
    ...
    def file_list(rootDir, ext_filter, percent):
        """
        Get list of .set files from ADFTD dataset
        - Skips derivatives folder to use only raw data
        ...
        for root, dirs, files in walker:
            for file in files:
                if not file.endswith('.set'):
                    continue
                # Skip derivatives folder to use only raw data
    ...
    def shaping(raw, original_sampling_rate, segment_len):
        ...
        Similar to HBN dataset processing
        """
    ...
    def qc(
        ...
        """
        Quality control with bad channel removal and clipping
        Same QC process as HBN and TUEG datasets
        """
        n_ch, n_sec, fs = sample.shape
        per_ch_size = n_sec * fs
        ...
        abs_sample = np.abs(sample)
        over_mask = abs_sample > threshold
        num_over = over_mask.sum(axis=(1, 2))

        # 2) bad channel 판정
        bad_ch_mask = num_over >= (per_ch_size *0.0333)
        n_bad = bad_ch_mask.sum()
...
_REGISTRY = {
        ...
        'ADFTD': dict(ext=".set",
            modality='EEG',
            loader=lambda x: mne.io.read_raw_eeglab(x, preload=True),
            channel_list=[f"E{i}" for i in range(1, 129)] + ["Cz"],  # GSN-HydroCel-129
            coordinate_function=utils._load_xyz_from_sfr,
            train_type="pretrain",
            split = None,
            ...
            qc_process=ADFTDDatasetSetting.qc,
